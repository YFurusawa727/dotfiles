...
- 🎨 **シェルカスタマイズ**: starshipプロンプトとfzfを統合した強化zsh
...
- `zsh-autosuggestions` - 履歴ベースの補完
- `zsh-syntax-highlighting` - コマンドのリアルタイム色付け
...
| ショートカット | 動作 |
|--------------|------|
| `Ctrl+R` | コマンド履歴をfzfでインタラクティブ検索 |
| `Ctrl+E` | 最近移動したディレクトリをfzfで選択して移動 |
| `Ctrl+T` | カレントディレクトリ以下のファイルをfzfで検索してコマンドラインに挿入 |
| `Option+C` | カレントディレクトリ以下のディレクトリをfzfで選択してcd |

> **Note**: `Option+C` はAlacrittyの `option_as_alt = "Both"` 設定が必要です（設定済み）
...
| `g b` | `git branch` |
| `g co` | `git checkout` |
| `g cob` | `git checkout -b` |
| `g cm` | `git commit -m` |
| `g pso` | `git push origin` |
| `g pl` | `git pull` |
| `g l` | `git log` |
...
```
 ~/dotfiles  main !2 ?1  took 3s          node v22
🦄 ❯
```

| 表示 | 意味 |
|------|------|
| ` ~/dotfiles` | 現在のディレクトリ（リポジトリルートは太字） |
| ` main` | 現在のブランチ名 |
| `!2` | 変更済みファイル数 |
| `?1` | 未追跡ファイル数 |
| `+1` | ステージ済みファイル数 |
...
| `⇡2` | リモートより2コミット先行 |
| `⇣1` | リモートより1コミット遅延 |
| `took 3s` | 2秒以上かかったコマンドの実行時間 |
| `node v22` | 使用中のNode.jsバージョン |
